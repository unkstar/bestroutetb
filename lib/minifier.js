...
function appendForceRoutes(rules, routes, gateway)
{
  routes.forEach(function(route) {
    var prefix = Prefix.parse(route);
    rules.push({
      prefix: prefix.toIPv4(),
      mask: prefix.toMask(),
...
function appendForceVPN(rules, routes)
{
  appendForceRoutes(rules, routes, 'vpn');
}

function appendForceNET(rules, routes)
{
  appendForceRoutes(rules, routes, 'net');
}
...
function minify(specs) {
  ...
  appendForceVPN(rules, specs.force_vpn);
  appendForceNET(rules, specs.force_net);
